...
const pushGlobalData = async (covidStats, globalSize, dataPushed) => {
  ...
  // else {
  //   const existingData = await GlobalModel.find();
  //   const existingDate = existingData[0].Date;
  //   const receivedDate = covidStats.Global.Date;
  //   if (Date(existingDate) === Date(receivedDate)) {
  //     const filter = { Date: Date(receivedDate) };
  //     const update = { TotalRecovered: 1000000 };
  //     const updatedData = await GlobalModel.findOneAndUpdate(filter, {
  //       $set: update,
